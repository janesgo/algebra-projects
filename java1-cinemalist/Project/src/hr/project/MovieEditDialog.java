 ...
 * @author Goran
 */
public class MovieEditDialog extends javax.swing.JDialog {
    ...
    public MovieEditDialog(java.awt.Frame parent, boolean modal, Movie movie) {
        super(parent, modal);
        this.movie = movie;
        initComponents();
        init();
    ...
    private void btnOkActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOkActionPerformed
        if (isFormValid()) {
            ...
            try {
                ...
                repository.movieRemoveAllGenres(movie.getId(), getUpdated(movie.getGenres(), genres));
                repository.movieAddAllGenres(movie.getId(), getUpdated(genres, movie.getGenres()));
                movie.setGenres(genres);

                repository.movieRemoveAllPeople(movie.getId(), getUpdated(movie.getActors(), actors), Position.from(Position.Actor));
                repository.movieAddAllPeople(movie.getId(), getUpdated(actors, movie.getActors()), Position.from(Position.Actor));
                movie.setActors(actors);

                repository.movieRemoveAllPeople(movie.getId(), getUpdated(movie.getDirectors(), directors), Position.from(Position.Director));
                repository.movieAddAllPeople(movie.getId(), getUpdated(directors, movie.getDirectors()), Position.from(Position.Director));
                movie.setDirectors(directors);

                repository.movieUpdate(movie.getId(), movie);
                dispose();
            } catch (IOException ex) {
                MessageUtils.showErrorMessage("Error", "Error saving poster");
            } catch (Exception ex) {
                MessageUtils.showErrorMessage("Error", "Error updating movie");
            }
        }
    ...
    private void formComponentShown(java.awt.event.ComponentEvent evt) {//GEN-FIRST:event_formComponentShown
        if (movie != null) {
            ...
            tfScreeningDate.setText(movie.getScreeningStartsDate()
            ...
            try {
                lblPoster.setIcon(IconUtils.createIcon(movie.getPosterPath(), 319, 355));
                tfPosterPath.setText(movie.getPosterPath());
            } catch (IOException ex) {
                MessageUtils.showErrorMessage("Error", "Error loading poster");
            }
        }
    ...
    private void initValidation() {
        validationFields = Arrays.asList(tfTitle, tfOriginalTitle, tfDuration, tfScreeningDate, tfPublishedDate, tfLink);
        errorLabels = Arrays.asList(lblTitleError, lblOriginalTitleError, lblDurationError, lblScreeningDateError, lblPublishedDateError, lblLinkError);
    }
    ...
    private void initGenresAndPeople() throws Exception {
        ...
        genres = new TreeSet<>(movie.getGenres());
        actors = new TreeSet<>(movie.getActors());
        directors = new TreeSet<>(movie.getDirectors());
    }
    ...
    private Set getUpdated(Set a, Set b) {
        Set c = new TreeSet<>(a);
        c.removeAll(b);
        return c;
    }
    ...
    private void initDragNDrop() {
        ...
        lsAllGenres.setTransferHandler(new MovieEditDialog.ExportHandler());

        lsGenres.setDropMode(DropMode.ON);
        lsGenres.setTransferHandler(new MovieEditDialog.ImportHandler());
    }
